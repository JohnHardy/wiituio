...
namespace WiiTUIO.WinTouch
{
    ...
    {
        #region WiiProvider Properties
        /// <summary>
        /// An internal reference to the WiiProvider.
        /// </summary>
        private WiiProvider pProvider = null;

        /// <summary>
        /// A reference to the provider we are using.
        /// </summary>
        public WiiProvider Provider
        {
            get
            {
            ...
            set
            {
                // If we have an existing provider - remove the event handler.
                if (this.pProvider != null)
                {
                    this.pProvider.OnNewFrame -= pEventHandler;
                }

                // Set the reference and add the event handler.
                this.pProvider = value;
                this.pProvider.OnNewFrame += pEventHandler;
            }
        }

        /// <summary>
        /// A reference to the event handler which we attach to providers.
        /// </summary>
        private EventHandler<FrameEventArgs> pEventHandler = null;
        #endregion
        ...
        {
            // Create the event handler to pass provider events to this class.
            this.pEventHandler = new EventHandler<FrameEventArgs>(Provider_OnNewFrame);
        ...
        /// <summary>
        /// This is called when we recieve a new frame of updates.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void Provider_OnNewFrame(object sender, FrameEventArgs e)
        {
            ...
            foreach (WiiContact pContact in e.Contacts)
            {
                ...
                switch (pContact.Type)
                {
                    case ContactType.Start:
                        this.enqueueContact(new HidContactInfo(HidContactState.Adding, pContact));
                        break;
                    case ContactType.Move:
                        this.enqueueContact(new HidContactInfo(HidContactState.Updated, pContact));
                        break;
                    case ContactType.End:
                        this.enqueueContact(new HidContactInfo(HidContactState.Removing, pContact));
                        break;
                }
            }
        }
        ...
        internal void enqueueContact(HidContactInfo pContact)
        {
